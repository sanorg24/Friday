 ...
 */
async function runCouncilCycle(previousState) {
  const userPrompt = previousState
    ? `Here is the current state of the business plan and prior decisions:\n\n${previousState}\n\nRun the next council cycle. Either continue executing the current plan's next step, or pivot if evidence suggests it, and explain why.`
    : `This is the first cycle. There is no existing business yet. Run the council and recommend a starting niche/idea to pursue with a $100 budget.`;
